...
function App() {
  ...
  const startIndex = Math.max(
    0,
    Math.floor(scrollTop / card_height) - buffer_cards,
  );
  const endIndex = Math.min(
    empData.length - 1,
    Math.floor((scrollTop + total_height) / card_height) + buffer_cards,
  );
  const visibleItems = empData.slice(startIndex, endIndex + 1);
  ...
  return (
    ...
    >
      <div style={{ position: "fixed", left: "45%", top: "5%", zIndex: "20" }}>
        ...
        >
          Nodes Rendered: {visibleItems.length} <br />
          Current Range: {startIndex} to {endIndex}
        </div>
      </div>
      <div
        style={{
